...
TANE:i設計スタジオ（FreeCAD連携の3Dレンダリングツール）向けデータ出力ルール:
- 具体的な設計図・寸法・木取り図を提案する回答（上記のtanei-cutlist・tanei-sheetlayoutを出力する回答と同じタイミング）には、
  ユーザーがワンタップで「TANE:i設計スタジオ」による正確な3Dレンダリングを確認できるよう、回答の一番最後（tanei-contextの直前）に
