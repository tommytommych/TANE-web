...
（Vercel等）上でFreeCADを実行しているわけではない）。

**この機能はPC専用機能と位置づけている**（クラウド常時稼働化は検討したが、
Renderへのデプロイ設定が繰り返し失敗し運用の複雑さに見合わないと判断し撤回した）。
オペレーターの手元PCで`python3 server.py`（既定ポート5002）を起動して使う
（下記「セットアップ」参照）。TANE:i側は常にこの`http://localhost:5002`へ接続する。
同じWi-Fi上のスマートフォン等からは、`/app/studio`の⚙️から接続先をPCのIPアドレス
（例: `192.168.1.23:5002`）に切り替えれば利用できる。
...
フォーム最上部の「家具プリセット選択」から、よく使う家具10種類（テレビ台・
オープンラック・ローテーブル・キッチンラック・本棚（スリム）・デスク・
シューズラック・壁面棚・サイドテーブル・収納ボックス）の代表的な寸法
（幅・奥行・高さ・板厚）をワンクリックで反映できる（`static/index.html`の
`FURNITURE_PRESETS`配列）。選択すると品名・寸法欄が更新され、既存の
`renderPayload(collectPayloadFromForm())`を呼び出して即座に再レンダリングする
（フォーム送信ボタンを別途押す必要がない）。プリセット一覧の`<option>`は
配列から動的に生成しているため、`FURNITURE_PRESETS`に項目を追加・編集するだけで
セレクトボックスにも自動的に反映される。
...
- **実機検証で発見・修正したバグ**: 新規接続時に直近の確定仕様を再送する処理が送信元を
  ...
  両方をエンドツーエンドで確認済み。
- 既知の制約: 同期状態はFlaskプロセス内メモリのみ（`sessionId`単位で分離済み。下記
  「複数人が同時に使うと設計内容が混ざる」参照）で、プロセス再起動で消える。また
  `server.py`は`host="0.0.0.0"`でLAN全体に公開されているため、同一Wi-Fi上の他端末からも
  正しい`sessionId`さえ分かれば仕様を送り込める状態になる。社内利用に限定するなら
  `127.0.0.1`に絞るか、`/ws/sync`接続時の認証を追加することを推奨する。
...
## 既知の制約・今後の検討事項
- ~~複数人が同時に使うと設計内容が混ざる~~ → **対応済み**。`server.py`の同期状態は
  `sessions`（sessionIdごとの`{"clients", "spec", "source"}`）に分離してあり、
  `renders/`配下も`renders/<sessionId>/<jobId>/`で分けている。sessionIdは
  チャット側（`app/lib/studioSession.ts`）がタブ単位で発行し、iframeのURLと
  WebSocket・レンダリングリクエストの全てに載せて渡す。詳細は`server.py`内の
  該当コメント参照。ただし同時レンダリングによる負荷（下記）は別問題として残る。
