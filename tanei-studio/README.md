...
## 既知の制約・今後の検討事項
- **［クラウド公開時・要対応］複数人が同時に使うと設計内容が混ざる**: `server.py`の
  `sync_clients` / `latest_spec`はプロセス全体でグローバル1つしかない簡易実装。
  不特定多数への本格的な一般公開の前に、セッションID単位（URLクエリやCookieで発行）で
  状態を分離する対応が必須。詳細は[DEPLOY.md](./DEPLOY.md)と`server.py`内の該当コメント参照。
