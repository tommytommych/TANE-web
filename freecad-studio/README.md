...
よう、小さい円柱（直径20〜60mm、高さ20mm固定）として表現し直した。木材ではなくゴム/樹脂の
タイヤなので、フィニッシュが"clear"でも他パーツのように木目にはせず、既定でダークグレーの
ゴム色（`WHEEL_FINISH_COLOR`/`WHEEL_FINISH_HEX`）になる（"walnut"を指定しても見た目上の
意味がないため同じゴム色にフォールバックする。"white"/"black"はそれぞれプラスチック風の
明るいグレー・より濃い黒になる）。パネル辞書に`"shape": "wheel"`を持たせることで、
以下の4箇所がそれぞれ「箱ではなく円柱として描く」分岐を持つ:

- `build_freecad_document()` — `Part.makeCylinder()`
- `generate_pov_scene()` — POV-Rayの`cylinder{}`プリミティブ
- Three.jsビューア（`buildViewerFromPanels()`） — `CylinderGeometry`
  （既定でY軸方向に高さを持つため、FreeCADのZ軸=高さと自然に対応し回転補正が不要）
